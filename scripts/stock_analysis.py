...
try:
    _result = {
        ...
        "analysts": {name: {"score": pos["score"], "signal": pos["signal"]} for name, pos in final_positions.items()},
        "weighted_score": round(weighted_sum, 4),
        "final_score": round(final_score, 4),
        "recommendation": rec,
        "consensus_signal": consensus.get("consensus_signal", "hold") if consensus else "hold",
        "confidence": round(consensus.get("confidence", final_score), 3) if consensus else round(final_score, 3),
        "target_prices": {
        ...
        "news_count": len(combined),
        "sentiment": sentiment_result["combined_label"],
        "debate_rounds": debate_result["rounds_completed"],
        "debate_messages": debate_result["total_messages"],
